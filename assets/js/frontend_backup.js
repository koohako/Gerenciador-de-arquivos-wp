...
jQuery(document).ready(function($) {
    console.log('🚀 Frontend JS carregado - versão com paginação');
    ...
    $('.gap-frontend-container').each(function() {
        ...
        console.log('📁 Inicializando container:', container, 'Base folder:', baseFolder);
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        currentPage = 1;
                        console.log('📊 Total de arquivos:', totalFiles, 'Arquivos:', allFiles);
                        displayCurrentPage();
                        updateNavigation();
                        updatePagination();
                    } else {
        ...
        function displayCurrentPage() {
            console.log('📄 Exibindo página:', currentPage, 'de', Math.ceil(totalFiles / itemsPerPage));
            const startIndex = (currentPage - 1) * itemsPerPage;
            const endIndex = startIndex + itemsPerPage;
            const pageFiles = allFiles.slice(startIndex, endIndex);
            console.log('🔢 Índices:', startIndex, '-', endIndex, 'Arquivos da página:', pageFiles);
                    ...
                    html += `
                        <div class="gap-file-item folder" data-name="${file.name}" data-type="folder">
                            ...
                            <div class="gap-file-info">
                                <div class="gap-file-name" title="${file.name}">${file.name}</div>
        ...
        function showError(message) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        displayFiles(response.data.items);
                        updateNavigation();
                    } else {
                        showError('Erro ao carregar arquivos: ' + response.data);
                    }
